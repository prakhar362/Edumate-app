                    ...
                    <SafeAreaView edges={['top']}>
                        ...
                        <View className="items-center mb-6">
                            <View className="w-52 h-52 bg-violet-800 rounded-3xl items-center justify-center shadow-2xl">
                                <LinearGradient colors={['#7c3aed', '#4c1d95']} className="w-full h-full rounded-3xl items-center justify-center">
                                    <Ionicons name="musical-notes" size={80} color="white" style={{ opacity: 0.85 }} />
                                </LinearGradient>
                            </View>
                        </View>
                        ...
                        {/* Playlist Info */}
                        <View>
                            <Text className="text-white text-3xl font-extrabold mb-2">{playlistInfo.title}</Text>
                            <View className="flex-row items-center justify-between">
                                <Text className="text-gray-300 text-base flex-1" numberOfLines={1}>{playlistInfo.description}</Text>
                                <View className="bg-white/10 px-3 py-1 rounded-full ml-4">
                                    <Text className="text-white text-sm font-semibold">{playlistInfo.count} items</Text>
                                </View>
                            </View>
                        </View>
                        ...
                        items.map((item, index) => (
                            <TouchableOpacity
                                ...
                                onPress={async () => {
                                    ...
                                    // 3. TRIGGER AUDIO PLAYBACK INSTEAD OF ROUTING
                                    if (targetId && item.audio_path) {
                                        await loadTrack({
                                            id: targetId,
                                            title: item.name || item.title || 'Untitled Item',
                                            subtitle: item.pdf_url ? 'PDF Source' : 'Audio Source',
                                            audioUrl: item.audio_path,
                                        });
                                        // The BottomPlayer component handles showing itself automatically.
                                    } else if (targetId) {
                                        // Fallback if no audio file exists
                                        router.push(`/summary/${targetId}`);
                                    }
                                }}
                                className="flex-row items-center py-4 mb-2 rounded-xl px-3 bg-white/5"
                            >
                                <View className="w-8 items-center mr-3">
                                    <Text className="text-gray-500 font-semibold">{index + 1}</Text>
                                </View>
                                <View className="flex-1">
                                    <Text className="text-white font-semibold text-base" numberOfLines={1}>{item.name || 'Untitled Item'}</Text>
                                    <View className="flex-row items-center mt-1">
                                        {item.score !== undefined && item.score > 0 && (
                                            <View className="bg-emerald-500/20 px-2 py-0.5 rounded mr-2">
                                                <Text className="text-emerald-400 text-xs font-bold">QUIZ {item.score}</Text>
                                            </View>
                                        )}
                                        <Text className="text-gray-400 text-xs">{item.pdf_url ? 'PDF Source' : 'Audio Source'}</Text>
                                    </View>
                                </View>
