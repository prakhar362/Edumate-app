...
export default function ProfileScreen() {
  ...
  const InfoTile = ({
    ...
    >
      ...
      {/* Edit Indicator (Visual Cue) */}
      {isEditing && (
        <Ionicons name="pencil" size={14} color="#ddd" />
      )}
    </View>
  );
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      {/* 1. Background Ambient Gradient (Subtle) */}
      <LinearGradient
        colors={["#f5f3ff", "#ffffff"]} // Very light violet to white
        style={{
          position: "absolute",
          left: 0,
          right: 0,
      ...
      <SafeAreaView className="flex-1" edges={['top']}>
        ...
        <View className="flex-row justify-between items-center px-6 pt-2 pb-2">
          ...
          <Text className="text-slate-900 text-2xl font-bold tracking-tight">My Profile</Text>

          <TouchableOpacity
            onPress={() => (isEditing ? handleSave() : setIsEditing(true))}
            disabled={loading}
            className={`px-4 h-10 rounded-2xl items-center justify-center flex-row shadow-sm ${
                isEditing ? "bg-violet-600 rounded-md" : "bg-white border border-slate-100"
            }`}
          >
             {loading ? (
                <ActivityIndicator size="small" color={isEditing ? "white" : "#7c3aed"} />
             ) : (
                 <>
                    <Text className={`font-bold text-xs ${isEditing ? "text-white" : "text-slate-700"} mr-1`}>
                        {isEditing ? "Save" : "Edit"}
                    </Text>
                    {!isEditing && <Ionicons name="settings-outline" size={14} color="#334155" />}
                 </>
             )}
          </TouchableOpacity>
        </View>
        ...
        >
          {/* 3. Avatar Section with Gradient Ring */}
            ...
            <View className="relative">
                ...
                >
                    <View className="bg-white p-1 rounded-full">
                        <Image
                            source={{ uri: "https://static.vecteezy.com/system/resources/previews/026/630/551/non_2x/profile-icon-symbol-design-illustration-vector.jpg" }}
                ...
                {isEditing && (
                    <TouchableOpacity className="absolute bottom-4 right-0 bg-slate-900 w-8 h-8 rounded-full items-center justify-center border-2 border-white shadow-lg">
          ...
          {/* 4. Floating Info Tiles */}
          <View className="mb-4">
            <Text className="text-slate-900 font-bold text-lg mb-4 ml-1">Personal Info</Text>
          ...
          {/* 5. Security Section */}
          <View className="mb-4">
            <Text className="text-slate-900 font-bold text-lg mb-4 ml-1">Security</Text>
            ...
            {isEditing ? (
                ...
                <TouchableOpacity 
                    ...
                    onPress={() => Alert.alert("Coming Soon", "Change Password flow")}
                >
                    ...
                    <View className="flex-1">
                        <Text className="text-slate-900 font-bold">Change Password</Text>
                        <Text className="text-slate-500 text-xs">Last updated 3 months ago</Text>
                    </View>
